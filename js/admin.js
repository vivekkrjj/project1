...
async function saveGallery(){
 await requireAdmin();
 const id=read('gallery_id'),title=read('gallery_title');let image_url=read('gallery_image');const file=$('gallery_file')?.files?.[0];
 if(!title)return alert('Title is required.');
 if(file){
...
async function refresh(){
 try{
  ...
  $('facultyAdminList').innerHTML=faculty.map(f=>`<div class="faculty faculty-admin-row"><img class="faculty-admin-photo" src="${esc(f.photo_url||'assets/bhumi-nursing-college-logo.png')}" alt="${esc(f.name)} photo"><div class="faculty-admin-info"><div><b>${esc(f.name)}</b><span>${esc(f.role||'')}</span><small>${esc(f.department||'')}</small></div><div class="cms-actions"><button class="outline-btn" data-edit-faculty="${f.id}">Edit</button><button class="delete-btn" data-fid="${f.id}">Delete</button></div></div></div>`).join('')||'<p>No faculty.</p>';
...
async function fillFacultyPhotoPreview(path){if(!path){$('facultyPhotoPreview').src='assets/bhumi-nursing-college-logo.png';return} $('facultyPhotoPreview').src=await getFacultyPhotoUrl(path)}
...
async function saveFaculty(){
 try{
  ...
  const file=$('facultyPhoto')?.files?.[0];
  if(file){if(file.size>5*1024*1024)return alert('Photo must be 5 MB or smaller.');if(!['image/jpeg','image/png','image/webp'].includes(file.type))return alert('Please select JPG, PNG or WEBP image.');}
...
async function getStudentPhotoUrl(path){try{const q=await bhumiDb.storage.from('student-profiles').createSignedUrl(path,1800);if(!q.error&&q.data?.signedUrl)return q.data.signedUrl}catch(e){}return 'assets/bhumi-nursing-college-logo.png'}
...
async function showStudentDetail(id){
 ...
 let photo='assets/bhumi-nursing-college-logo.png';
...
function renderLibraryAdminList(){
 ...
 const course=read('lib_filter_course'), year=read('lib_filter_year'), sem=read('lib_filter_semester'), subject=read('lib_filter_subject'), type=read('lib_filter_type');
 const data=all.filter(i=>(!course||i.course===course)&&(!year||i.year_level===year)&&(!sem||i.semester===sem)&&(!subject||i.subject===subject)&&(!type||i.material_type===type));
 const box=$('libraryList');
 const status=$('libraryFilterStatus');
 if(status)status.textContent=`Showing ${data.length} of ${all.length} materials${subject?' · Subject: '+subject:''}${course?' · Course: '+course:''}`;
 box.innerHTML=data.map(i=>`<div class="cms-item"><div><b>${esc(i.course)} · ${esc(i.title)}</b><small>${esc(i.year_level||'')} ${i.semester?'· '+esc(i.semester):''} ${i.subject?'· '+esc(i.subject):'· All Subjects'} · ${esc(i.material_type)}</small></div><div class="cms-actions"><button class="outline-btn" data-lib-edit="${i.id}">Edit</button><button class="outline-btn danger" data-lib-del="${i.id}">Delete</button></div></div>`).join('')||'<p>No materials match the selected filters.</p>';
...
bindCMS();
if(isDbReady())bhumiDb.auth.getSession().then(async({data})=>{if(data.session){try{await requireAdmin();$('adminLogin').classList.add('hidden');$('adminDashboard').classList.remove('hidden');await loadCMS()}catch(e){await bhumiDb.auth.signOut()}}});
})();