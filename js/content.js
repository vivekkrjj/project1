(function(){
  ...
  const fallbackItems=[
    {item_type:'course',title:'B.Sc Nursing',description:'Four-year undergraduate nursing program with theory, skills lab and clinical training.',subtitle:'4 Years',icon:'🎓',link_url:'course-details.html#bsc',sort_order:1,is_active:true},
    {item_type:'course',title:'GNM',description:'Three-year General Nursing and Midwifery program focused on practical patient care.',subtitle:'3 Years',icon:'⚕',link_url:'course-details.html#gnm',sort_order:2,is_active:true},
    {item_type:'course',title:'ANM',description:'Two-year Auxiliary Nurse Midwifery program for foundational nursing practice.',subtitle:'2 Years',icon:'📜',link_url:'course-details.html#anm',sort_order:3,is_active:true},
...
{item_type:'feature',title:'B.Sc Nursing',description:'4 Years',icon:'🎓',sort_order:1,is_active:true},
  ...
  window.renderDynamicHome=async function(){
    try{
      ...
      if(cg)cg.innerHTML=courses.length?courses.map(i=>`<article class="card"><div class="icon">${escapeHtml(i.icon||'🎓')}</div><h3>${escapeHtml(i.title)}</h3><p>${escapeHtml(i.description||'')}</p><small>${escapeHtml(i.subtitle||'')}</small>${i.link_url?`<a href="${safeUrl(i.link_url,'#courses')}">View Details →</a>`:''}</article>`).join(''):'<div class="loading">No courses published yet.</div>';
