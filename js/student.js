...
async function loadStudentProfile(user,reg){
  ...
  $('studentProfilePhoto').src=path ? (await signedProfileUrl(path)) : 'assets/bhumi-nursing-college-logo.png';
...
async function signedProfileUrl(path){
  ...
  return 'assets/bhumi-nursing-college-logo.png';
}
...
async function removeStudentPhoto(){
  ...
  if(old) await bhumiDb.storage.from('student-profiles').remove([old]); await bhumiDb.auth.updateUser({data:{student_photo_path:null}}); $('studentProfilePhoto').src='assets/bhumi-nursing-college-logo.png'; msg.textContent='Profile photo removed.';
}
async function login(){
 const err=$('studentError');err.textContent='';
 if(!isDbReady()){err.textContent='Supabase is not configured yet.';return}
 const email=$('studentEmail').value.trim(), password=$('studentPassword').value;
...
async function register(){
 ...
 const existing=await bhumiDb.from('student_registration_requests').select('id,status').ilike('email',normalizedEmail).limit(1).maybeSingle();
 if(existing.error){err.textContent=existing.error.message;return}
 if(existing.data){
   err.textContent=existing.data.status==='approved'?'This email is already registered and approved. Please use Student Login.':existing.data.status==='pending'?'This email already has a pending registration. Please wait for admin approval.':'This email is already registered. Please contact the college office.';
   return;
 }
 const {data,error}=await bhumiDb.auth.signUp({email:normalizedEmail,password,options:{data:{full_name,role:'student'}}});
 if(error){err.textContent=error.message;return}
 if(!data.user){err.textContent='Unable to create account. Please try again.';return}
 if(!data.user.identities || data.user.identities.length===0){err.textContent='This email is already registered. Please use Student Login.';return}
 const row={user_id:data.user.id,full_name,email:normalizedEmail,mobile,dob,course,roll_number,status:'pending'};
 const ins=await bhumiDb.from('student_registration_requests').insert(row);
 if(ins.error){err.textContent=ins.error.code==='23505'?'This email is already registered. Please use Student Login.':ins.error.message;return}
